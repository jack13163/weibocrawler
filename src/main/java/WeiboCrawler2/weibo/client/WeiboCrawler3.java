...
public class WeiboCrawler3 {
    ...
    public String getWeiboSentence(String iniWeibo) {
        ...
        char cUTF8;
        String cString;
        Pattern p = Pattern.compile("ff1a<em>.+?<\\\\/em>");
    ...
    public String getPraisedNum(String iniWeibo) {
        Pattern ppraised = Pattern.compile("W_ico20 icon_praised_b.+?<\\\\/a>");
    ...
    public String getUserName(String iniWeibo) {
        ...
        Pattern puserName = Pattern.compile("<a nick-name.+?href");
    ...
    public Vector<String> getWeiboInfo(String htmlString) {
        ...
        String oneIniWeibo;
        Pattern poneIniWeibo = Pattern.compile("feed_list W_linecolor.+?W_ico20 icon_praised_b.+?feed_list_item_date");//根据微博搜索页面html的特点，只有当前微博有赞，转发的那条内嵌微博不呈现赞
        Pattern pforwardContent = Pattern.compile("feed_list_forwardContent");//是否转发自某条微博
        ...
        while (moneIniWeibo.find()) {
            ...
            if (mforwardContent.find()) {//去除掉转发的那条微博
                oneIniWeibo = oneIniWeibo.replaceAll("<dt node-type=\\\\\"feed_list_forwardContent\\\\\">.+?<p class=\\\\\"info W_linkb W_textb\\\\\">", "");
            }
    ...
    public void crawl(int totalPage, String today, String searchword) throws ClientProtocolException, URISyntaxException, IOException, InterruptedException {
        ...
        for (int i = totalPage; i > 0; i--) {
            ...
            html = crawler.getHTML("http://s.weibo.com/weibo/" + searchword + "&nodup=1&page=" + String.valueOf(i));
            ...
            if (html != "html获取失败") {
                if (crawler.isVerification(html)) {
                    ...
                    crawler.runBroswer("http://s.weibo.com/weibo/%25E6%259D%258E%25E9%259B%25AA%25E5%25B1%25B1hakka&Refer=index");
                    Thread.sleep(10000);//你有十秒时间可以填入验证码
                    i++;
                } else if (crawler.isExistResult(html)) {
                    crawler.writeString(html, "Data/" + today + "/" + searchword + "/" + searchword + String.valueOf(i) + ".html");
                    System.out.println("获取第" + i + "页成功！");
                } else
                    System.out.println("第" + i + "页内容不存在");
            } else {
                //如果因为连接超时问题，获得的html为"html获取失败"的话，则再重新连接一次
                i++;
            }
        }
    }
    ...
    public static void main(String[] args) throws ClientProtocolException,
        ...
        String basePath = "Data/" + today + "/";
        File dirGetweiboSub = new File(basePath);
        dirGetweiboSub.mkdirs();
        for (int n = 0; n < searchwords.length; n++) {
            String searchword = searchwords[n];
            File f = new File(basePath + searchword);
            f.mkdirs();//创建文件夹，另一方法mkdirs创建多层未创建的文件夹

            String saveWeibosTXTPath = basePath + searchword + "/" + searchword + n + ".txt";
            String saveWeibosXMLPath = basePath + searchword + "/" + searchword + n + ".xml";
            ...
            for (int i = 1; i < totalPage + 1; i++) {
                String htmlPath = basePath + searchword + String.valueOf(i) + ".html";
